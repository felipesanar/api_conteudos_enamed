...
swaggerui_blueprint = get_swaggerui_blueprint(
    SWAGGER_URL,
    API_URL,
    config={'app_name': "API Cronograma de Estudos (Estrutura de Dicionário)"}
)
...
def criar_chave_semana(semana_str):
    """
    Cria uma chave única e limpa para a semana (ex: "Semana 01 (...)" -> "semana_1").
...
def extrair_area_conhecimento(dia_str):
    """Extrai a área de conhecimento do nome do dia (ex: "Dia 1 - GO" -> "GO")"""
    if '-' in dia_str:
        return dia_str.split('-', 1)[1].strip()
    return ""
...
def processar_arquivos_para_hierarquia(pasta_arquivos):
    ...
    """
    # A estrutura agora é um dicionário onde a chave é 'semana_1', 'semana_2', etc.
    ...
    for arquivo in arquivos:
        try:
            ...
            for _, row in df.iterrows():
                ...
                # Pula a linha se não conseguir gerar uma chave para a semana
                if not chave_semana:
                    continue

                dia_str = row.get('Dia', '').strip()
                tema_str = row.get('Tema Principal', '').strip()
                subtema_str = row.get('Subtema', '').strip()
                aula_str = row.get('Aula', '').strip()

                if not dia_str or not tema_str or not subtema_str:
                    continue
                ...
                dia_obj["area_conhecimento"] = extrair_area_conhecimento(dia_str)

                tema_obj = dia_obj["temas"][tema_str]
                tema_obj["nome"] = tema_str
                ...
                if aula_str:
                    aula_nova = {
                        ...
                        "link_questoes": row.get('Link Questões', '').strip()
                    }
...
def formatar_cronograma_final(dados_brutos):
    ...
    for chave_semana in chaves_ordenadas:
        ...
        dias_lista = []
        # Ordena os dias pelo nome (ex: "Dia 1", "Dia 2")
...
def home():
    return """
    <h1>API Cronograma de Estudos (Estrutura de Dicionário)</h1>
    <p>A API agora retorna um dicionário de semanas para acesso direto.</p>
    <p>Exemplo de chave: <code>/api/cronograma</code> -> <code>{"cronograma": {"semana_1": {...}}}</code></p>
...
def buscar():
    ...
    parameters:
      - name: q
        ...
        type: string
        required: true
        description: Termo a ser buscado (ex: 'pré-natal', 'ginecologia', 'semana 1').
    responses:
      200:
            ...
            for tema in dia.get("temas", []):
                for subtema in tema.get("subtemas", []):
                    for aula in subtema.get("aulas", []):
                        ...
                        caminho_completo = f"{chave_semana} {semana['nome_exibicao']} {dia['nome']} {tema['nome']} {subtema['nome']} {aula['nome']}".lower()
